...
RESULT_FORMAT_GUIDE = """\
응답 스타일과 필드 작성 규칙:
- summary는 전체 위험을 1~2문장으로 요약한다. "총 N개" 같은 숫자를 억지로 만들지 말고, 핵심 위험 유형을 묶어 쓴다.
- sentence는 계약서 원문의 위험 문장을 그대로 옮긴다. 너무 길면 핵심 문장 1~2개만 포함한다.
- category는 4~12자 정도의 짧은 위험 유형으로 쓴다. 예: "전입신고/확정일자", "원상회복/보증금 공제", "연장근로수당".
- severity는 반드시 "high", "medium", "low" 중 하나만 쓴다.
- reason은 왜 위험한지 1~2문장으로 쓴다. 계약서 표현과 RAG 근거를 연결하되, 단정적인 법률 자문처럼 쓰지 않는다.
- evidence는 반드시 사용한 RAG 근거의 제목과 출처를 포함해 "[제목] / 출처: 출처명" 형식으로 쓴다. 여러 근거를 쓰면 세미콜론으로 구분한다.
- recommendation은 계약서에서 무엇을 고치거나 확인해야 하는지 1~2문장으로 구체적으로 쓴다.

좋은 출력 예시:
{
  "summary": "임차인의 대항력 확보를 제한하고 보증금 반환을 지연시킬 수 있는 조항이 포함되어 있습니다. 전입신고·확정일자 제한, 원상회복 부담, 중도해지 위약금 조건은 우선 검토가 필요합니다.",
  "risk_clauses": [
    {
      "sentence": "임차인은 임대인의 사전 서면 동의 없이 전입신고나 확정일자를 받을 수 없다.",
      "category": "전입신고/확정일자",
      "severity": "high",
      "reason": "전입신고나 확정일자 취득을 제한하면 임차인의 대항력과 보증금 회수 가능성에 영향을 줄 수 있습니다. 계약 위반이나 즉시 해지 사유로 두는 표현은 임차인에게 불리하게 작동할 수 있습니다.",
      "evidence": "[확정일자와 우선변제권] / 출처: 주택임대차보호법 제3조의2; [전입신고와 대항력 보호] / 출처: 주택임대차보호법 제3조",
      "recommendation": "전입신고와 확정일자 취득을 금지하거나 임대인 동의에 묶는 문구를 삭제하고, 임차인이 관련 절차를 진행할 수 있음을 명확히 하는 방향으로 수정하세요."
    },
    {
      "sentence": "임차인은 퇴거 시 통상적인 사용으로 생긴 마모나 노후화까지 포함하여 모든 시설을 신품 상태로 원상회복해야 한다.",
      "category": "원상회복/보증금 공제",
      "severity": "high",
      "reason": "통상적인 사용으로 생긴 마모까지 임차인에게 신품 수준으로 회복하게 하면 과도한 부담이 될 수 있습니다. 보증금에서 임의로 공제할 수 있다는 문구도 정산 범위를 불명확하게 만듭니다.",
      "evidence": "[원상회복과 통상 마모 구분] / 출처: 민법 제615조",
      "recommendation": "임차인의 고의·과실로 생긴 훼손과 통상 마모를 구분하고, 공제 항목·산정 기준·증빙 절차를 계약서에 구체화하세요."
    }
  ]
}
"""
